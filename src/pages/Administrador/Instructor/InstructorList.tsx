...
const InstructorList: React.FC = () => {
  ...
  const filteredInstructors = instructors.filter(instructor =>
    (instructor.usuario_username?.toLowerCase() || '').includes(searchTerm.toLowerCase()) ||
    (instructor.nombre?.toLowerCase() || '').includes(searchTerm.toLowerCase()) ||
    (instructor.apellido_paterno?.toLowerCase() || '').includes(searchTerm.toLowerCase()) ||
    (instructor.especialidad?.toLowerCase() || '').includes(searchTerm.toLowerCase())
  );

  if (loading) return <div className="p-6 text-center text-gray-500">Cargando instructores...</div>;
            ...
            {filteredInstructors.map((instructor) => (
              ...
              >
                <div className="absolute top-3 right-3 flex gap-1 z-10">
                  <Link
                    to={`/administrador/instructores/${instructor.usuario}`}
                ...
                <div className="flex items-center space-x-3 mb-4">
                  <div className="w-10 h-10 bg-slate-200 rounded-full flex items-center justify-center text-slate-600 font-medium text-xs">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-500 truncate">@{instructor.usuario_username}</p>
                  </div>
                </div>
